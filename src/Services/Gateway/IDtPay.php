...
{
    public function MetronPay($type, $price, $shop, $paylist_id=0)
    {
        if ($paylist_id == 0 && $price <= 0) {
            return ['ret' => 0, 'msg' => "非法的金额."];
        }

        if ( isset($shop['telegram']) ) {
            $user = $shop['telegram']['user'];
        } else {
            $user = Auth::getUser();
        }
        ...
        if ($paylist_id == 0) {
            ...
            if ($shop) {
                if ( isset($shop['telegram']) ) {
                    unset($shop['telegram']['user']);
                }
                $pl->shop   = json_encode($shop);
            }
            $pl->save();
        } else {
        ...
        $notify_url     = $_ENV['baseUrl'] . "/payment/notify/idtpay";
        $return_url     = $_ENV['baseUrl'] . "/user/payment/return";
        if ( isset($shop['telegram']) ) 
        $return_url     = 'https://t.me/' . $_ENV['telegram_bot'];
